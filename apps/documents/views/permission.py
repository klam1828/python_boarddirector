...
class PermissionDetailView(SelectBoardRequiredMixin, GetMembershipMixin,
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        def _cmp(p):
            if p.membership:
                return (1, p.membership.get_full_name())
            else:
                return (0, p.role)

        context['view_permissions'] = sorted(view_permissions.values(), key=_cmp)
        context['edit_permissions'] = sorted(edit_permissions.values(), key=_cmp)
...
class PermissionAddView(AjaxableResponseMixin, SelectBoardRequiredMixin, GetMembershipMixin,
    ...
    def form_valid(self, form):
        ...
        downloadable = form.cleaned_data.get('downloadable', None)
        permission = form.cleaned_data['permission']
        if memberships or (role and role.isdigit() and int(role) != 0):
            perm_obj = self.get_permission_object()
            self.do_add_permissions(memberships, permission, role, perm_obj)
        if downloadable is not None:
            obj = self.get_permission_object()
            if 'downloadable' in dir(obj):  # isinstance(perm_obj, Document):
                obj.downloadable = downloadable
                obj.save()  # update_fields['downloadable',]  # ??
