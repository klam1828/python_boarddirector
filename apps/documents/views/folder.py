...
class FolderContextMixin(object):
    ...
    def sort_items(items, ordering):
        ...
        if 'name' in ordering:
            ...
            items = sorted(items, key=lambda i: (i.ordering) * (2**16) + i.id)
        return items
...
class FolderDetailView(ActiveTabMixin, SelectBoardRequiredMixin,
                       FolderContextMixin, FolderQuerysetMixin,
...
class RootFolderDetailView(ActiveTabMixin, SelectBoardRequiredMixin, FolderContextMixin,
                           PermissionMixin, LoginRequiredMixin, DetailView):
    permission = (Folder, PERMISSIONS.view)
    active_tab = 'folders'
    template_name = 'documents/folder_detail.html'
