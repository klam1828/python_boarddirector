...
class FolderManager(TreeManager, models.Manager):
    ...
    def get_discussion_folder(self, account):
        discussion_folder, created = self.get_or_create(
            name='Discussion files',
            account=account,
            level=1,
            parent=self.get_account_root(account),
            hidden=True,
            special_field=self.model.SPECIAL_FIELDS.discussion
        )

        if created:
            self._share_folder(discussion_folder)
            self._share_folder_write(discussion_folder)
    ...
    def _share_folder(self, folder, permission=False):
    ...
    def _share_folder_write(self, folder):
        from permissions import PERMISSIONS
        from permissions.models import ObjectPermission
        from permissions.utils import get_contenttype
        from profiles.models import Membership
