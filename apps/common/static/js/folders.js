...
function download_disabled_alert(){
    // show confirmation dialog    
    var dda = $('#alert-download-disabled');
    dda.dialog('open');
    $('.popup-overlay').show();
}
...
$(document).ready(function() {
    ...
    $(document).on('click', 'a.download-disabled', function (event) {
        event.preventDefault();
        download_disabled_alert();
    });

    var downloadAlert = $('#alert-download-disabled');

    downloadAlert.dialog({
        autoOpen: false,
        width: 696,
        resizable: false,
        buttons: [
    ...
    if (!$sort.length) {
        ...
        $sort.click(function () {
            ...
            $('.sortingLinks').css('left', $(this).position().left - 76).toggle();
            return false;
        });
    }
