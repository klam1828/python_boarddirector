...
class LoginRequiredMixin(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # check: enforced 2FA, social login permissions
        rdir = None
        if settings.T2FA_ALLOW_ENFORCED:
            rdir = self.check_enforced_2fa(request)
        if rdir is None:
            rdir = self.check_social_allowed(request)
        if rdir is not None:
            return rdir

        return super(LoginRequiredMixin, self).dispatch(request, *args, **kwargs)

    def check_social_allowed(self, request):
        """ deny social logins if admin disabled it for the account """
        iam_social = 'from_social_login' in request.session
        account = get_current_account(request)
        user = request.user

        if iam_social and user and account is not None and not account.enable_social_logins:
            return redirect('forbidden', reason="social")

        return None

    def check_enforced_2fa(self, request):
        """ if admin enforced 2FA for the account, 
        but the user does not have configured 2FA, then redirect to 2FA setup page.
        """
        account = get_current_account(request)
        user = request.user
        iam_social = 'from_social_login' in request.session

        if not iam_social and user and account is not None and account.enforce_2fa:
        # if Membership.objects.filter(user=user, account__enforce_2fa=True).exists():
            try:
                if user.is_verified():
                    return None
                elif user.totpdevice_set.count():
                    # 2fa enabled, OTP device configured, but user not verified
                    # -- wierd, redirect to login page
                    return redirect('two_factor:login')
                else:
                    # 2fa enabled, but OTP not configured
