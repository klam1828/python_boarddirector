...
from .forms import AccountReactivateForm, AccountLogo, AccountNotify, AccountSaveSettings
...
from common.mixins import (ActiveTabMixin, LoginRequiredMixin, CurrentAccountMixin,
                           ...
                           AccountGetObjectMixin, GetDataMixin, LoginRequiredMixin)
...
class MembersView(ActiveTabMixin, SelectBoardRequiredMixin, MembershipQuerysetMixin,
                  PermissionMixin, LoginRequiredMixin, ListView):
...
class ExportMembersPdfView(GetDataMixin, AccountGetObjectMixin, SelectBoardRequiredMixin,
                           MembershipQuerysetMixin, PermissionMixin, LoginRequiredMixin, View):
...
class ExportMembersXlsView(GetDataMixin, SelectBoardRequiredMixin, MembershipQuerysetMixin,
                           PermissionMixin, LoginRequiredMixin, View):
    permission = (Membership, PERMISSIONS.add)
    model = Membership
...
class AssistantCreateView(CurrentAccountMixin, ActiveTabMixin, MemberCreateMixin, SelectBoardRequiredMixin,
                          MembershipQuerysetMixin, PermissionMixin, LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.account.send_invitations:
...
class MemberCreateView(CurrentAccountMixin, ActiveTabMixin, RecentActivityMixin, MemberCreateMixin, MemberNotificationMixin,
                       SelectBoardRequiredMixin, MembershipQuerysetMixin, PermissionMixin, LoginRequiredMixin, CreateView):
...
class AccountSettingsView(SelectBoardRequiredMixin, AccountGetObjectMixin,
                          PermissionMixin, LoginRequiredMixin, DetailView):
    permission = (Account, PERMISSIONS.view)
    template_name = 'accounts/account_settings.html'


class AccountCancelView(AjaxableResponseMixin, SelectBoardRequiredMixin, PermissionMixin, LoginRequiredMixin, View):
...
class AccountSaveSettingsView(AjaxableResponseMixin, AccountGetObjectMixin,
                              PermissionMixin, LoginRequiredMixin, BaseUpdateView):
    permission = (Account, PERMISSIONS.edit)
    form_class = AccountSaveSettings
    ...
    def form_valid(self, form):
        ...
        if 'enforce_2fa' in form.changed_data and not self.object.enforce_2fa \
            ...
            for member in self.object.memberships.all():
                u = member.user
                logger.warn("deleting 2FA configuration for user `%s`: totp=%i phones=%i bak=%i"
                            % (member.user.email, u.totpdevice_set.count(), u.phonedevice_set.count(), u.staticdevice_set.count()))
