...
from common.mixins import GetMembershipMixin, SelectBoardRequiredMixin, ActiveTabMixin, \
    GetMembershipWithURLFallbackMixin, LoginRequiredMixin
from permissions.mixins import PermissionMixin
from voting.models import Voting


class VotingMainView(ActiveTabMixin, SelectBoardRequiredMixin, PermissionMixin,
                     GetMembershipMixin, LoginRequiredMixin, TemplateView):
...
class VotingVoteView(TemplateView, GetMembershipWithURLFallbackMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_anonymous:
            # Side-effect this will set current account by url for this request, makes acc_url and other things work fine.
